...
def get_model(num_users, num_items, layers=[20, 10], reg_layers=[0, 0]):
    ...
    MLP_Embedding_User = Embedding(input_dim=num_users, output_dim=int(layers[0] / 4), name='user_embedding',
                                   embeddings_regularizer=l2(reg_layers[0]), input_length=1)
    MLP_Embedding_Item = Embedding(input_dim=num_items, output_dim=int(layers[0] / 4), name='item_embedding',
                                   embeddings_regularizer=l2(reg_layers[0]), input_length=1)

    conv_user = Conv1D(filters=32, kernel_size=2, padding='same')(MLP_Embedding_User(user_input))
    conv_item = Conv1D(filters=32, kernel_size=2, padding='same')(MLP_Embedding_Item(item_input))
    # Crucial to flatten an embedding vector!
    user_latent = Flatten()(conv_user)
    item_latent = Flatten()(conv_item)
